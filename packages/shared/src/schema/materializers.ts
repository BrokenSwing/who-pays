...
export const appMaterializers = materializers(events, {
  ...
  "v1.GroupRenamed": ({ id, name }) => tables.groups.update({ name }).where({ id }),
  ...
  "v1.MemberRenamed": ({ id, name }) => tables.members.update({ name }).where({ id }),

  "v1.MemberRemoved": ({ id }) => tables.members.update({ deletedAt: new Date() }).where({ id }),
  ...
  "v1.ExpenseUpdated": ({
    ...
    const ops:
      | ReturnType<typeof tables.expenses.update>[]
      | ReturnType<typeof tables.expense_splits.insert>[] = [];
  ...
  "v1.ExpenseDeleted": ({ id }) => tables.expenses.update({ deletedAt: new Date() }).where({ id }),
